...
(function () {
  ...
  var START_POSITION = fenToObj(START_FEN); // default animation speeds

  var DEFAULT_APPEAR_SPEED = 200;
  var DEFAULT_MOVE_SPEED = 200;
  var DEFAULT_SNAPBACK_SPEED = 60;
  var DEFAULT_SNAP_SPEED = 30;
  var DEFAULT_TRASH_SPEED = 100; // use unique class names to prevent clashing with anything else on the page
  ...
  function pieceCodeToFen(piece) {
    var pieceCodeLetters = piece.split(''); // white piece
  ...
  function expandConfig(config) {
    ...
    if (config.orientation !== 'black') config.orientation = 'white'; // default for showNotation is true

    if (config.showNotation !== false) config.showNotation = true; // default for draggable is false

    if (config.draggable !== true) config.draggable = false; // default for dropOffBoard is 'snapback'

    if (config.dropOffBoard !== 'trash') config.dropOffBoard = 'snapback'; // default for sparePieces is false

    if (config.sparePieces !== true) config.sparePieces = false; // draggable must be true if sparePieces is enabled

    if (config.sparePieces) config.draggable = true; // default piece theme is wikipedia
    ...
    if (!validAnimationSpeed(config.trashSpeed)) config.trashSpeed = DEFAULT_TRASH_SPEED; // throttle rate
  ...
  function constructor(containerElOrString, config) {
    ...
    if (!$container) return null; // ensure the config object is what we expect

    config = expandConfigArgumentShorthand(config);
    config = expandConfig(config); // DOM elements

    var $board = null;
    var $draggedPiece = null;
    var $sparePiecesTop = null;
    var $sparePiecesBottom = null; // constructor return object
    ...
    function setInitialState() {
      currentOrientation = config.orientation; // make sure position is valid
    ...
    function calculateSquareSize() {
      var containerWidth = parseInt($container.width(), 10); // defensive, prevent infinite loop
    ...
    function animateSquareToSquare(src, dest, piece, completeFn) {
      ...
      var destSquarePosition = $destSquare.offset(); // create the animated piece and absolutely position it
      ...
      function onFinishAnimation1() {
        ...
        $destSquare.append(buildPieceHTML(piece)); // remove the animated piece

        $animatedPiece.remove(); // run complete function
    ...
    function animateSparePieceToSquare(piece, dest, completeFn) {
      ...
      var $destSquare = $('#' + squareElsIds[dest]);
      var destOffset = $destSquare.offset(); // create the animate piece
    ...
    function doAnimations(animations, oldPos, newPos) {
      ...
      function onFinishAnimation3() {
        ...
        drawPositionInstant(); // run their onMoveEnd function
      ...
      for (var i = 0; i < animations.length; i++) {
        var animation = animations[i]; // clear a piece

        if (animation.type === 'clear') {
          $('#' + squareElsIds[animation.square] + ' .' + CSS.piece).fadeOut(config.trashSpeed, onFinishAnimation3); // add a piece with no spare pieces - fade the piece onto the square
        } else if (animation.type === 'add' && !config.sparePieces) {
          $('#' + squareElsIds[animation.square]).append(buildPieceHTML(animation.piece, true)).find('.' + CSS.piece).fadeIn(config.appearSpeed, onFinishAnimation3); // add a piece with spare pieces - animate from the spares
        } else if (animation.type === 'add' && config.sparePieces) {
          animateSparePieceToSquare(animation.piece, animation.square, onFinishAnimation3); // move a piece from squareA to squareB
    ...
    function drawPositionInstant() {
      // clear the board
      $board.find('.' + CSS.piece).remove(); // add the pieces
    ...
    function snapbackDraggedPiece() {
      ...
      function complete() {
        drawPositionInstant();
        $draggedPiece.css('display', 'none'); // run their onSnapbackEnd function
      ...
      var sourceSquarePosition = $('#' + squareElsIds[draggedPieceSource]).offset(); // animate the piece to the target square

      var opts = {
        duration: config.snapbackSpeed,
        complete: complete
      };
      $draggedPiece.animate(sourceSquarePosition, opts); // set state
    ...
    function trashDraggedPiece() {
      ...
      $draggedPiece.fadeOut(config.trashSpeed); // set state
    ...
    function dropDraggedPieceOnSquare(square) {
      ...
      var targetSquarePosition = $('#' + squareElsIds[square]).offset(); // animation complete

      function onAnimationComplete() {
        drawPositionInstant();
        $draggedPiece.css('display', 'none'); // execute their onSnapEnd function
      ...
      $draggedPiece.animate(targetSquarePosition, opts); // set state
    ...
    function beginDraggingPiece(source, piece, x, y) {
      ...
      isDragging = true;
      draggedPiece = piece;
      draggedPieceSource = source; // if the piece came from spare pieces, location is offboard
    ...
    function updateDraggedPiece(x, y) {
      ...
      if (location === draggedPieceLocation) return; // remove highlight from previous square
    ...
    function stopDraggedPiece(location) {
      ...
      if (isFunction(config.onDrop)) {
        var newPosition = deepCopy(currentPosition); // source piece is a spare piece and position is off the board
    ...
    widget.destroy = function () {
      // remove markup
      $container.html('');
      $draggedPiece.remove(); // remove event handlers
    ...
    widget.move = function () {
      ...
      var newPos = calculatePositionFromMoves(currentPosition, moves); // update the board
    ...
    widget.position = function (position, useAnimation) {
      ...
      if (useAnimation) {
        ...
        doAnimations(animations, currentPosition, position); // set the new position
    ...
    widget.resize = function () {
      ...
      squareSize = calculateSquareSize(); // set board width

      $board.css('width', squareSize * 8 + 'px'); // set drag piece size
    ...
    function mousedownSquare(evt) {
      ...
      if (!config.draggable) return; // do nothing if there is no piece on this square
    ...
    function touchmoveWindow(evt) {
      ...
      if (!isDragging) return; // prevent screen from scrolling
    ...
    function mouseupWindow(evt) {
      // do nothing if we are not dragging a piece
      if (!isDragging) return; // get the location
    ...
    function mouseenterSquare(evt) {
      ...
      if (isDragging) return; // exit if they did not provide a onMouseoverSquare function

      if (!isFunction(config.onMouseoverSquare)) return; // get the square

      var square = $(evt.currentTarget).attr('data-square'); // NOTE: this should never happen; defensive

      if (!validSquare(square)) return; // get the piece on this square
    ...
    function mouseleaveSquare(evt) {
      // do not fire this event if we are dragging a piece
      // NOTE: this should never happen, but it's a safeguard
      if (isDragging) return; // exit if they did not provide an onMouseoutSquare function

      if (!isFunction(config.onMouseoutSquare)) return; // get the square
    ...
    function addEvents() {
      ...
      $container.on('mousedown', '.' + CSS.sparePieces + ' .' + CSS.piece, mousedownSparePiece); // mouse enter / leave square

      $board.on('mouseenter', '.' + CSS.square, mouseenterSquare).on('mouseleave', '.' + CSS.square, mouseleaveSquare); // piece drag

      var $window = $(window);
      $window.on('mousemove', throttledMousemoveWindow).on('mouseup', mouseupWindow); // touch drag pieces
    ...
    function initDOM() {
      ...
      boardBorderSize = parseInt($board.css('borderLeftWidth'), 10); // set the size and draw the board
    ...
    setInitialState();
    initDOM();
    addEvents(); // return the widget object
  ...
  window['Chessboard'] = constructor; // support legacy ChessBoard name

  window['ChessBoard'] = window['Chessboard']; // expose util functions
...
},{}]},{},["../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","chessboard-1.0.0.js"], null)
